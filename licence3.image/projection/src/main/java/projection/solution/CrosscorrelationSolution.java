...
@Plugin(type = Command.class, menuPath = "Plugins>TD 4 Solution>Correlate images (Solved)")
public class CrosscorrelationSolution<T extends RealType<T>> implements Command {
	...
	public double correlate(RandomAccess<T> imageCursor, long[] imageDimensions, RandomAccess<T> templateCursor,
		...
		double div = templateDimensions[0] * templateDimensions[1]; // Pour normaliser

		//
		for (long i = 0; i < templateDimensions[0]; i++) {
			positionTemplate[0] = i;
			positionImage[0] = currentImagePosition[0] - midPositionTemplate[0] + i;
			for (long j = 0; j < templateDimensions[1]; j++) {
				positionTemplate[1] = j;
				positionImage[1] = currentImagePosition[1] - midPositionTemplate[1] + j;

				if (positionImage[0] >= 0 && positionImage[1] >= 0 && positionImage[0] < imageDimensions[0]
						&& positionImage[1] < imageDimensions[1]) {
					templateCursor.setPosition(positionTemplate);
					imageCursor.setPosition(positionImage);

					sum += (templateCursor.get().getRealDouble() * imageCursor.get().getRealDouble()) / div;
				}
			}
		}
